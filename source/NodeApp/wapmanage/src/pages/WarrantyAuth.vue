  ...
  <div>
        ...
        <mt-tab-container v-model="tabindex">
            ...
            <mt-tab-container-item :id="2">
               <div class="search" v-show="!nodataflag1">
                <input type="text" placeholder="输入材质、炉批号、手机号查询" v-model="condition1">
                ...
                <div class="opened" v-for="(item,index1) in list1" :key="index1">
                    <h3 class="opened_itemtitle">{{item.sellername}}({{item.mobile}})</h3>
...
function loadOpenedPage (vue, condition1) {
  var datalist = []
  vue.$http.get('api/v1/SellerOpenedAuth/?PageSize=' + openedPostData.PageSize + '&PageIndex=' + openedPostData.PageIndex + '&condition=' + condition1).then(function (res) {
    if (res.status === 0) {
      datalist = []
      vue.list1 = datalist
...
function loadOpenPage (vue, condition) {
  vue.$http.get('api/v1/SellerAuth/?PageSize=' + openPostData.PageSize + '&PageIndex=' + openPostData.PageIndex + '&condition=' + condition).then(function (res) {
    if (res.status === 0) {
      vue.$toast({ message: '没有更多数据了', position: 'bottom' })
      vue.list = []
      vue.nodataflag = true
...
export default {
  ...
  methods: {
    ...
    searchClidk1: function () {
      loadOpenedPage(this, this.condition1)
    },
    searchClidk: function () {
      loadOpenPage(this, this.condition)
    },
    auth: function () {
      var vue = this
