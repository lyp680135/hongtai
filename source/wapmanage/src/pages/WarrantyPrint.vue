  ...
  <div>
        ...
        <mt-tab-container v-model="tabindex">
            ...
            <mt-tab-container-item :id="2">
              <div class="search">
                <input type="text" placeholder="输入打印编号或收货地址查询" v-model="condition">
                ...
                <!-- <mt-loadmore :top-method="loadTop1" :bottom-method="loadBottom1" :bottom-all-loaded="disallowBottomLoaded1" ref="loadmore1"> -->              
                    <mt-cell v-for="(item,index2) in list1" :key="index2" is-link  @click.native="onCellClick(item)">
                        <div slot="title" class="iteminfo">
                            <div class="bianhao">
                              <p>{{item.Consignor}}
                                  <br/>
...
function loadOpenedPage (vue, condition) {
  var datalist = []
  vue.$http
    .get(
      'api/v1/WarrantyPrint/?PageSize=' +
        openedPostData.PageSize +
        '&PageIndex=' +
...
function loadOpenPage (vue, condition) {
  vue.$http
    .get(
      'api/v1/SellerAuth/?PageSize=' +
        openPostData.PageSize +
        '&PageIndex=' +
  ...
  methods: {
    ...
    minus: function (index, index1) {
      if (this.list[index].list[index1].SelectedNumber > 0) {
        this.list[index].list[index1].SelectedNumber =
          parseInt(this.list[index].list[index1].SelectedNumber) - 1
        totalNum(this)
      }
    },
    calc: function (index, index1) {
      try {
        this.list[index].list[index1].SelectedNumber = parseInt(
          this.list[index].list[index1].SelectedNumber
        )
        var num =
          this.list[index].list[index1].Number -
          this.list[index].list[index1].AuthNumber

        if (this.list[index].list[index1].SelectedNumber > num) {
          this.$toast({
            message: '授权件数不能大于剩余件数',
            position: 'bottom'
          })
    ...
    searchClidk () {
      loadOpenedPage(this, this.condition)
    },
    searchClidk1 () {
      loadOpenPage(this, this.condition1)
    },
    cancalAuth: function (authid) {
      var vue = this
      vue.$messagebox
        .confirm('确认撤消此经销商及下级所有经销商对此条物资的权限吗？')
        .then(function (action) {
          if (action === 'confirm') {
            var postdata = { authids: authid }
            vue.$http
              .put('api/v1/SellerAuth/?requestData=' + JSON.stringify(postdata))
              .then(function (res) {
                if (res.status === 1) {
                  vue.$toast({
                    message: '撤消授权成功',
    ...
    genWarranty: function () {
      ...
      if (msg.length > 0) {
        this.$toast({
          message: '同品名、同牌号、同类型的物资才能生成质保书，请重新选择',
          position: 'bottom'
        })
        return
      }
