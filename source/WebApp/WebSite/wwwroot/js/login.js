        ...
        $.ajax({
            ...
            success: function (re) {
                if (re.status) {
                    ...
                    // window.location.href = reData.redirect + "SetAuth?s=" + reData.access_token + "&con=" + reData.system_config;
            ...
            $.ajax({
                ...
                success: function (re) {
                    if (re.status) {
                        ...
                        if (systemMemberType.val() == 1) {
                            layer.alert(re.msg, { icon: 2 });
                        } else {
                            layer.alert("该手机号不存在于系统中", { icon: 2 });
                        }
                    }
                    layer.close(index);
                }, error: function () {
                            ...
                            if (systemMemberType.val() == 0) {
                                ...
                                setTimeout(function () {
                                    layer.open({
                                        ...
                                        shade: 0.8,
                                        area: ['320px', '250px'],
                                        content: '/Login/SetPwd' + "?s=" + reData.access_token //iframe的url
                                    });
                                }, 1500);
    ...
    $(document).keyup(function (event) {
        if (event.keyCode == 13) {
            if ($(".layui-layer-iframe").length < 1) {
                $("#btnLogin_ByUserName").trigger("click");
                $("#btnLogin_ByPhone").trigger("click");
            }
            $("#btn_SetPwd").trigger("click");
