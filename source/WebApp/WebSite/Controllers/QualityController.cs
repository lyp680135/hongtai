    ...
    public class QualityController : Controller
    {
        ...
        public string GetPatCodeNew(string curcode, int offset, int wid)
        {
            ...
            if (current != null)
            {
                ...
                if (offset > 0)
                {
                    productInfo = this.db.PdBatcode.Where(w => w.Workshopid == wid && w.Workshopline == current.Workshopline).OrderBy(o => o.Serialno).FirstOrDefault(f => f.Serialno > serialno);
                }
                else
                {
                    productInfo = this.db.PdBatcode.Where(w => w.Workshopid == wid && w.Workshopline == current.Workshopline).OrderByDescending(o => o.Serialno).FirstOrDefault(f => f.Serialno < serialno);
                }
                                ...
                                for (int i = 0; i < 3; i++)
                                {
                                    ...
                                    duoxList.ForEach(s =>
                                    {
                                        if (dicHead.Keys.Any(w => w == s.TargetName))
                                        {
                                            ...
                                            {
                                                val = value.ToDouble();
                                                if (s.TargetName.StartsWith("伸长率"))
                                                {
                                                    val = value.ToDouble(1);
                                                }
                                            }
                                            if (s.TargetName.StartsWith("伸长率"))
                                            {
                                                keyValue.Add(s.TargetName, val.ToString("f1"));
                                            }
                                            else
                                            {
